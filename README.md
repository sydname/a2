...
| 탭 | 내용 | 출처 |
|---|---|---|
| 기본 | 아이템레벨 · 전투력 · 오드 · 주간 체크박스(회랑/일회/각성 · 루드라/침식/무스펠/비탄/시련) | API + 수동 |
| 강화 | 기본데바니온(네자칸+지켈+바이젤+트리니엘) · 유스티엘 · 상위 스티그마 6 | API 자동 |
| 돌파 | 부위별 돌파 단계(색상) · 팬던트 강화 | API 자동 |
| 잠재력 | 부위별 잠재력 · 유일/영웅 · 티어 (칸 색 = 그 부위 아이템 등급색) | `docs/manual.json` (엑셀값) |
| 영혼각인 | 방어구 7부위 각인 옵션(subStats) | 아르카나 워크플로 (수동 갱신) |

- 행 순서는 항상 `characters.json` 순서. 표 정렬 기능은 없습니다.
- 데바니온 보드 ID 는 직업마다 달라서 **보드 이름**으로 매칭합니다.

## 동작 방식 / 참고

- 수집은 공식 홈페이지가 내부적으로 쓰는 JSON API 를 그대로 호출합니다 (로그인 불필요):
  - `GET /api/character/info` — 프로필 · 스탯 · 아이템레벨 · 칭호 · 데바니온 보드
  - `GET /api/character/equipment` — 장비/돌파 · 아르카나 · 스티그마(스킬) · 날개 · 펫
  - `GET /api/character/equipment/item` — 아이템 상세(아르카나 서브스킬, 방어구 영혼각인) — 수동 워크플로 전용
